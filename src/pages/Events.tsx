...
const Events = () => {
  ...
  const getCalendarDays = () => {
    ...
    // Преобразуем день недели (0=Вс, 1=Пн, ..., 6=Сб) в формат (0=Пн, 1=Вт, ..., 6=Вс)
    const adjustedDayOfWeek = startDayOfWeek === 0 ? 6 : startDayOfWeek - 1;
    
    // Добавляем пустые ячейки для дней до начала месяца
    const emptyDays = Array(adjustedDayOfWeek).fill(null);
